...
class ConectaLanaDirectorio {
    ...
    async _fetchRows() {
        ...
        // B4 (jul 2026): batchGet — Registro + Raw_backup (el raw de Tally vive allí desde el 29/06;
        // lo necesitamos para los "servicios" de los transformadores).
        const response = await this.sheets.spreadsheets.values.batchGet({
            spreadsheetId: SPREADSHEET_ID,
            ranges: [`${SHEET_NAME}!A:Z`, 'Raw_backup!A:E'],
        });
        const vranges = response.data.valueRanges || [];
        const values = vranges[0] && vranges[0].values;
        if (!values || values.length < 2) return null;

        // mapa email|tipo → raw (la última alta gana)
        const rawMap = {};
        const rawValues = (vranges[1] && vranges[1].values) || [];
        for (let i = 1; i < rawValues.length; i++) {
            const rr = rawValues[i] || [];
            if (rr[3] && rr[4]) rawMap[`${deaccent(rr[3])}|${deaccent(rr[1])}`] = rr[4];
        }
        ...
        for (let i = 1; i < values.length; i++) {
            ...
            // B4: fuera del directorio los dados de baja, duplicados y filas de prueba
            const estadoMatch = deaccent(get('estado_match'));
            if (estadoMatch.startsWith('fuera') || estadoMatch.includes('duplicado') || estadoMatch.includes('prueba')) continue;
            if (deaccent(get('nombre')).includes('prueba')) continue;
            ...
            // B4: campos normalizados (columnas S:V del Registro) para los filtros del directorio
            entry.ccaa = get('ccaa_norm');
            entry.provincia = get('provincia_norm');
            entry.uso = get('uso_norm');
            entry.kg = get('kg_norm');
            // badge "Nuevo" — alta de los últimos 7 días
            entry.esNuevo = false;
            const f = get('fecha');
            if (f) {
                const d = new Date(f.replace(' ', 'T'));
                if (!isNaN(d.getTime())) entry.esNuevo = (Date.now() - d.getTime()) < 7 * 24 * 3600 * 1000;
            }

            if (tipo === 'transformo') {
                // el raw ya no vive en Registro: lo recuperamos de Raw_backup por email+tipo
                const rawStr = get('raw') || rawMap[`${deaccent(get('email'))}|${deaccent(get('tipo'))}`] || '';
                const pairs = this._parseRaw(rawStr);
