    ...
    async _fetchRows() {
        ...
        for (let i = 1; i < values.length; i++) {
            ...
            const entry = {
                tipo,
                nombre,
                zona: get('provincia_zona') || get('zona') || get('provincia'),
                cantidad: get('cantidad'),
                raza_o_busca: get('raza_o_busca') || get('raza') || get('busca'),
                estado_lana: get('estado_lana') || get('estado'),
                para_que: get('para_que'),
            };

            if (tipo === 'transformo') {
                const pairs = this._parseRaw(get('raw'));
                entry.servicios = get('servicios')
                ...
                entry.tercerosExtra = (terceros && !isYes(terceros) && deaccent(terceros) !== 'no') ? terceros : '';
            }
